...
frappe.query_reports["Purchase Analytics"] = {
    filters: [
        ...
        },
        {
            ...
            reqd: 1,
            change: function () {
                // Show/hide custom_purchase_type filter depending on doc_type
                const customFilter = frappe.query_report.get_filter("custom_purchase_type");
                if (this.get_value() === "Purchase Order") {
                    customFilter.df.hidden = 0; // show filter
                } else {
                    customFilter.df.hidden = 1; // hide filter
                    customFilter.set_value(""); // reset
                }
