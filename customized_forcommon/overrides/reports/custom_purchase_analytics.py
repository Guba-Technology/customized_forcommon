...
def execute(filters=None):
	return CustomPurchaseAnalytics(filters).run()


class CustomPurchaseAnalytics:
	...
	def get_columns(self):
		...
		# Add custom purchase type column when based on Purchase Order
		if self.filters.doc_type == "Purchase Order":
			self.columns.append(
				{
	...
	def get_data(self):
		if self.filters.tree_type in ["Customer", "Supplier"]:
			self.get_transactions_based_on_customers_or_suppliers()
			self.get_rows()

		elif self.filters.tree_type == "Item":
			self.get_transactions_based_on_items()
			self.get_rows()

		elif self.filters.tree_type in ["Customer Group", "Supplier Group", "Territory"]:
			self.get_transactions_based_on_customer_or_territory_group()
			self.get_rows_by_group()

		elif self.filters.tree_type == "Item Group":
			self.get_transactions_based_on_item_group()
			self.get_rows_by_group()
			...
			if self.filters.doc_type != "Sales Order":
				self.data = []
				return
			self.get_transactions_based_on_order_type()
			self.get_rows_by_group()

		elif self.filters.tree_type == "Project":
			self.get_transactions_based_on_project()
			self.get_rows()

	def get_transactions_based_on_order_type(self):
		...
		# Add custom purchase type filter for Purchase Order
		if self.filters.doc_type == "Purchase Order" and self.filters.get("custom_purchase_type"):
			query = query.where(doctype.custom_purchase_type == self.filters.custom_purchase_type)

		self.entries = query.run(as_dict=True)
		self.get_teams()

	def get_transactions_based_on_customers_or_suppliers(self):
		if self.filters["value_quantity"] == "Value":
		...
		# Add custom purchase type filter for Purchase Order
		if self.filters.doc_type == "Purchase Order" and self.filters.get("custom_purchase_type"):
			filters["custom_purchase_type"] = self.filters.custom_purchase_type
	...
	def get_transactions_based_on_items(self):
		if self.filters["value_quantity"] == "Value":
	...
	def get_transactions_based_on_customer_or_territory_group(self):
		if self.filters["value_quantity"] == "Value":
			value_field = "base_net_total as value_field"
		else:
			value_field = "total_qty as value_field"
	...
	def get_transactions_based_on_item_group(self):
		if self.filters["value_quantity"] == "Value":
			value_field = "base_net_amount"
		else:
			value_field = "qty"
	...
	def get_transactions_based_on_project(self):
		if self.filters["value_quantity"] == "Value":
			value_field = "base_net_total as value_field"
		else:
			value_field = "total_qty as value_field"
	...
	def get_rows(self):
		...
		for entity, period_data in self.entity_periodic_data.items():
			...
			# Add custom purchase type if available in period_data
			if "custom_purchase_type" in period_data:
				row["custom_purchase_type"] = period_data.get("custom_purchase_type")
	...
	def get_rows_by_group(self):
		...
		for d in reversed(self.group_entries):
			...
			# Add custom purchase type if available in period_data
			if d.name in self.entity_periodic_data and "custom_purchase_type" in self.entity_periodic_data[d.name]:
				row["custom_purchase_type"] = self.entity_periodic_data[d.name].get("custom_purchase_type")
	...
	def get_periodic_data(self):
		...
		for d in self.entries:
			...
			# Store custom purchase type if available
			if hasattr(d, 'custom_purchase_type'):
				self.entity_periodic_data[d.entity]["custom_purchase_type"] = d.custom_purchase_type
	...
	def get_period(self, posting_date):
		if self.filters.range == "Weekly":
			...
			year = get_fiscal_year(posting_date, company=self.filters.company[0] if isinstance(self.filters.company, list) else self.filters.company)
			period = str(year[0])
		return period
	...
	def get_chart_data(self):
		...
		if self.filters.tree_type in ["Customer", "Supplier"]:
			start_idx = 2
			# Adjust for purchase type column
			if self.filters.doc_type == "Purchase Order":
				start_idx = 3
			labels = [d.get("label") for d in self.columns[start_idx:length - 1]]
		elif self.filters.tree_type == "Item":
			start_idx = 2
			if self.filters.doc_type == "Purchase Order":
				start_idx = 3
